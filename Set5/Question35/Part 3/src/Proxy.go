...
/*************************G=P-1*******************/
/*When G = P-1, G would always be even. Because P would always be odd. 2 is the only even prime.*/
...
func parametersFromClient(w http.ResponseWriter, r *http.Request){
	...
	proxyEncKey = DHE.GenerateSymmetricKey(&g,big.NewInt(2),&publicExp)//Key Generation
	return
}
