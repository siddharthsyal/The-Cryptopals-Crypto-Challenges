...
func generateSymmetricKey(pubAlice,pubBob,secretBob,secretAlice,p *big.Int){
	alice_key := squareANDmultiply(p,secretAlice,p)
	bob_key := squareANDmultiply(p,secretBob,p)
	if alice_key.Cmp(bob_key)==0{
		fmt.Println("Successful DHE")
	}
	if alice_key.Cmp(big.NewInt(int64(0)))==0{
		fmt.Println("Due to (P^something) mod P the encryption key becomes 0")
		fmt.Printf("\n%x",getHash(alice_key))
	}
	return
}
