...
The attack on secret-prefix SHA1 relies on the fact that you can take the output of SHA-1 and use it as a new starting point for SHA-1, thus taking an arbitrary SHA-1 hash and "feeding it more data".
...
Source for SHA1 implementation : https://github.com/delaemon/go-sha1/blob/master/sha1.go

The above mentioned SHA1 implementation has been modified and used the challenge29.go source file to accommodate the challenge needs.

For Windows users, run the code using the following command : go run sha1.go challenge29.go